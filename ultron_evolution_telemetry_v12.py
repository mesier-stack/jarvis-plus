...
def _load():
    try:
        ...
        return {'generation_metrics':{},'last_diff':{},'auto_reverts':0}
...
def record_generation_sample(generation:int,success:bool,latency_ms:float,error:bool=False):
    d=_load()
    g=d['generation_metrics'].setdefault(
        str(generation),
        {'samples':0,'success':0,'errors':0,'latency_total':0.0,'fitness':0.0}
    )
    g['samples']+=1
    g['success']+=int(bool(success))
    g['errors']+=int(bool(error))
    g['latency_total']+=max(0.0,float(latency_ms))

    rate=g['success']/max(1,g['samples'])
    err=g['errors']/max(1,g['samples'])
    avg=g['latency_total']/max(1,g['samples'])
    latency_score=max(0.0,10.0-min(10.0,avg/300.0))
    raw_fitness=(rate*70.0)+((1.0-err)*20.0)+latency_score
    g['fitness']=round(max(0.0,min(100.0,raw_fitness)),2)
    _save(d)
    return g['fitness']
...
def set_diff(before,after):
    ...
    d['last_diff']={'ts':time.time(),'changes':diff}
    _save(d)
...
def maybe_auto_revert():
    ...
    if cur.get('samples',0)<8 or prev.get('samples',0)<8:
        return False,''
    if float(cur.get('fitness',0))+8 < float(prev.get('fitness',0)):
        ok,msg=rollback_generation()
        if ok:
            d['auto_reverts']=int(d.get('auto_reverts',0))+1
            _save(d)
            return True,'AUTO-REVERT // generation degraded beyond threshold. '+msg
    return False,''

def install_evolution_telemetry_v12():
    if getattr(UltronBrain,'_evo_telemetry_v12',False):
        return
    ...
    def handle(self,raw):
        ...
        if low in {'evolution diff','rewiring diff','what changed'}:
            d=_load()
            return AssistantReply('EVOLUTION DIFF // '+json.dumps(d.get('last_diff',{}),indent=2),'status')
        return old(self,raw)
    UltronBrain.handle=handle
    UltronBrain._evo_telemetry_v12=True
