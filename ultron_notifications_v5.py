...
def install_notifications_v5(app_cls):
    ...
    def init(self,*a,**k):
        self._ultron_notifications=deque(maxlen=80)
        old_init(self,*a,**k)
        self.bind("<Control-n>",lambda _e:self._open_notifications_v5())
    def append(self,speaker,text,tag="assistant"):
        result=old_append(self,speaker,text,tag)
        q=getattr(self,"_ultron_notifications",None)
        if q is None:
            q=deque(maxlen=80); self._ultron_notifications=q
        if speaker=="SYSTEM" or tag in {"error","status"}:
            q.appendleft((datetime.now().strftime("%H:%M:%S"),speaker,str(text)[:240],tag))
        return result
    def open_notifications(self):
        ...
        items=list(getattr(self,"_ultron_notifications",()))
        if not items: ctk.CTkLabel(body,text="No system notices yet.",text_color=MUTED).pack(pady=30)
        for t,s,msg,tag in items:
